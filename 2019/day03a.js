...
var x = 0;
var y = 0;
path1.forEach( function( path ) {
	coords1 = draw( path, coords1 );
} );

x = 0;
y = 0;
path2.forEach( function( path ) {
	coords2 = draw( path, coords2 );
} );

// Calculate Manhatten Distance
var manhattenDistance = 0;

coords1.forEach( function( wire1 ) {
	coords2.forEach( function( wire2 ) {
		// do the wires cross?
		if ( wire1.x == wire2.x && wire1.y == wire2.y ) {
			console.log( "cross", wire1.x, wire1.y );
			var distance = Math.abs( wire1.x ) + Math.abs( wire1.y );
			// is this closer than the last recorded distance?
			if ( manhattenDistance == 0 || manhattenDistance > distance ) {
...
console.log( "manhattenDistance",manhattenDistance );


function draw( path, coords ) {
	var direction = path.substring( 0, 1 );
	var distance = parseInt( path.substring( 1, path.length ) );

	switch( direction ) {
		case "R": // right
			for ( var d = x; d <= x + distance; d++ ) {
				if ( !( d == 0 && y == 0 ) ) {
					coords.push( { x: d, y: y } );
				}
			}
